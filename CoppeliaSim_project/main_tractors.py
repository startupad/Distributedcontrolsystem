...
from CoppeliaSim_project.main import load_processed_matrix, find_value_coordinates, create_straight_path, \
...
class Tractor:
    def __init__(self, sim):
        ...
        self.starting_config = [1, 1, 0.3, 0.00013, -0.7, 0.00036, 0.7]
    ...
    def load_tractor_model(self):
        ...
        if not os.path.exists(path_tract):
            ...
            if self.tractor_handle == -1:
                ...
                self.point_b_handle = self.sim.createPrimitiveShape(self.sim.primitiveshape_spheroid, [0.1, 0.1, 0.1],
                                                                    0)
    ...
    def calculate_new_path(self, input_path):
        ...
        plot_file = os.path.join(os.path.dirname(__file__), 'interpolated_path.png')
    ...
    def next_animation_step(self, t_step):
        ...
        cos_theta = np.dot(actual_pos[0:2], self.config_to_reach) / (
                np.linalg.norm(actual_pos[0:2]) * np.linalg.norm(self.config_to_reach))
